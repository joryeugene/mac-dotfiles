...
.PHONY: all install discover brew casks cli configs set_permissions backup_configs help manual_installs update check_dependencies compare_cursor_profiles sync_cursor_profiles check_outdated clean health_check list_installed update_npm node_check discover_cursor_extensions
...
configs:
	...
	# Install Cursor extensions if needed
	@if [ -f "$(DOTFILES_DIR)/cursor_extensions/extensions.txt" ]; then \
		echo "Installing Cursor extensions..."; \
		if command -v $(CURSOR_CLI) >/dev/null 2>&1; then \
			cat $(DOTFILES_DIR)/cursor_extensions/extensions.txt | while read extension; do \
				echo "Installing extension: $$extension"; \
				$(CURSOR_CLI) --install-extension "$$extension" || echo "Failed to install $$extension"; \
			done; \
		else \
			echo "Cursor CLI not found, cannot automatically install extensions"; \
			echo "Please install extensions manually from $(DOTFILES_DIR)/cursor_extensions/extensions.txt"; \
		fi; \
	fi
